...
pnpm og           # rebuild the social card + favicon from the data (needs the geodata build)
```
...
chromium already installed for the a11y tests, so it adds no dependency, and skips cleanly when the
geodata builds are absent.
