...
def calculate_environmental_parameters_for_cooling_technologies(
    ...
    carbon_intensity_electric_grid_kgCO2eq_kWh = global_parameters[
        "carbon_intensity_electric_grid_kgCO2eq_kWh"
    ]  # Carbon intensity of the electric grid in kg CO2-eq per kWh
    ...
    df_cooling_technologies["SEER_inv"] = (
        1 / df_cooling_technologies["SEER"]
    )  # The inverse SEER is the amount of electricity needed to produce 1 unit of cooling

    # Calculate the greenhouse gas (GHG) emissions in kg CO2-eq per kWh cooling energy demand
    df_cooling_technologies["GHG_emissions_electricity_kgCO2eq_kWh_cooling"] = (
        carbon_intensity_electric_grid_kgCO2eq_kWh * df_cooling_technologies["SEER_inv"]
    )

    # Calculate the effective annual refrigerant leakage in kg per kW of installation power
    df_cooling_technologies["refrigerant_leakage_kg_kW"] = (
        df_cooling_technologies["refrigerant_leakage_rate_relative"]
        * df_cooling_technologies["refrigerant_density_kg_kW"]
    )

    # Calculate the annual climate change impact from refrigerant leakage in kg CO2-eq per kW of installation power
    df_cooling_technologies["GHG_emissions_refrigerant_leaks_kgCO2eq_kW"] = (
        gwp_refrigerant_kgCO2eq_kg * df_cooling_technologies["refrigerant_leakage_kg_kW"]
    )
    ...
    df_cooling_technologies["GHG_emissions_production_phase_kgCO2eq_kW"] = (
        carbon_intensity_production_kgCO2eq_kg
        * df_cooling_technologies["material_density_kg_kW"]
        / df_cooling_technologies["average_lifetime_yr"]
    )
...
def calculate_impacts_from_cooling_energy_demand(buildings: pd.DataFrame, cap_percentile: int = 98) -> pd.DataFrame:
    ...
    buildings["GHG_emissions_electricity_kgCO2eq"] = (
        cooling_energy_demand_kWh * buildings["avg_GHG_emissions_electricity_kgCO2eq_kWh_cooling"]
    )
...
def calculate_impacts_from_peak_cooling_power_demand(buildings: pd.DataFrame, cap_percentile: int = 98) -> pd.DataFrame:
    ...
    buildings["GHG_emissions_refrigerant_leaks_kgCO2eq"] = (
        peak_cooling_power_demand_kW * buildings["avg_GHG_emissions_refrigerant_leaks_kgCO2eq_kW"]
    )
    ...
    buildings["GHG_emissions_production_phase_kgCO2eq"] = (
        peak_cooling_power_demand_kW * buildings["avg_GHG_emissions_production_phase_kgCO2eq_kW"]
    )
    ...
    buildings["GHG_emissions_EoL_phase_kgCO2eq"] = (
        peak_cooling_power_demand_kW * buildings["avg_GHG_emissions_EoL_phase_kgCO2eq_kW"]
    )
...
def create_impact_summary(
    ...
    return {
        "total_impacts": {
            ...
            "P_cooling_demand_peak_percentile_MW": round(
                buildings[f"P_cooling_peak_{cap_percentile}th_percentile_kW"].sum() / 1_000,
                5,
            ),  # Total peak cooling power demand in MW
            "electricity_use_total_GWh": round(
                buildings["electricity_use_kWh"].sum() / 1_000_000,
                5,
            ),  # Total electricity use in GWh
            "GHG_emissions_total_tonneCO2eq": round(
                buildings["GHG_emissions_total_kgCO2eq"].sum() / 1_000,
                5,
            ),  # Total GHG emissions in tonne CO2-eq
            "material_use_total_tonne": round(
                buildings["mass_cooling_equipment_kg"].sum() / 1_000,
                5,
            ),  # Total material use in tonne
        },
        "impacts_per_floor_area": {
            ...
            "P_cooling_peak_percentile_W_m2": round(
                ...
                5,
            ),  # Peak cooling power demand intensity in W/m2
            "electricity_use_intensity_kWh_m2": round(
                buildings["electricity_use_intensity_kWh_m2"].mean(),
                5,
            ),  # Electricity use intensity in kWh/m2
            "GHG_emissions_intensity_kgCO2eq_m2": round(
                buildings["GHG_emissions_intensity_kgCO2eq_m2"].mean(),
                5,
            ),  # GHG emissions intensity in kg CO2eq/m2
            "material_use_intensity_kg_m2": round(
                buildings["material_use_intensity_kg_m2"].mean(),
                5,
            ),  # Material use intensity in kg/m2
        },
    }
...
def calculate_environmental_impacts_from_cooling_demand(
    ...
    Returns:
        tuple[pd.DataFrame, dict[str, dict[str, float]]]: The buildings DataFrame with the environmental impacts depending on cooling energy and power demand added, and an impact summary for sensitivity analysis.
    """
    # Unload global parameters
    cap_percentile = int(
        global_parameters["peak_cooling_percentile_cap"],
    )  # The percentile at which the cooling energy and power demand is capped
    ...
    buildings["material_use_intensity_kg_m2"] = (
        buildings["mass_cooling_equipment_kg"] / buildings["floor_area_total_m2"]
    )
    buildings["GHG_emissions_intensity_kgCO2eq_m2"] = (
        buildings["GHG_emissions_total_kgCO2eq"] / buildings["floor_area_total_m2"]
    )
